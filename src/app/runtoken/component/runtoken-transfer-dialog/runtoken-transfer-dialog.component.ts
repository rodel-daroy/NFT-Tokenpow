...
@Component({
  selector: 'app-runtoken-transfer-dialog',
  templateUrl: './runtoken-transfer-dialog.component.html',
  styleUrls: ['./runtoken-transfer-dialog.component.scss']
})
export class RuntokenTransferDialogComponent implements OnInit {
  bsvAddress;
  isBrowser;
  promise = null;
  $subs: Subscription[] = [];
  ...
  onSubmit(answer: boolean): void {
    ...
    for (var i  = 0; i < this.config.data.decimals; i++) {
      amount *= 10
    }
    const response = {
      answer:answer,
  ...
  async onPaymentSuccess(event): Promise<void> {
      ...
      this.runtokenServices.transferTokenDirectly(this.location, this.ownerAddress, amount).then(
        res => {
          if (res?.success == true)
          {
