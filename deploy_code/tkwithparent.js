...
class TokenPowBaseNFT extends Jig {
  init(
    name,
    author,
    description,
...
TokenPowBaseNFT.metadata = {
  emoji: "👨‍🎨️",
};

TokenPowBaseNFT.interactive = false;
...
class TokenPowNFT extends TokenPowBaseNFT {
  init(name,
    author,
    description,
    emoji,
...
async function deploy() {
  ...
  tx.update(() => {
    ...
    TokenPowNFT.deps = {TokenPowBaseNFT};


    TokenPowNFT.friends = [TokenPowBaseNFT, orderlock, B];
    TokenPowBaseNFT.friends = [TokenPowNFT];
...
async function main() {
  ...
  for (let index = startEditionNo; index <= endEditionNo; index++) {
    setTimeout(() => {
      ...
      create(index, index, "4c5b9f12cb2627709908280b591bd1cf0c3c49ab224bb91d67de822d2800cea6_o2", "1AHNHiGWk8NJDjF2ZvCekA1eSsyfXbToWD");
    }, 5000);
  }
